...
pavlovia <-
    list.files(path = "/Users/andreadioni/Desktop/SocKult eksamen/Bachelor-Analysis/finalpavdata",
               pattern="*.csv", 
               full.names = T) %>% 
...
df <- subset(df, !is.na(df$RT)) %>% #remove missing reaction times
  ...
  #subset(HC_time == "Under 3 måneder") %>% #try to fix this
  subset(Age_1 < 40) %>% 
  subset(Gender != "Mand") %>% 
  subset(id !=675) %>% 
  subset(id!=3716) %>% 
...
df$HC_type[df$HC_type == "Hvis andet, hvad:"] <- "P-ring"
...
Er lidt i tvivl om vi reelt skal gøre det på den måde
```{r}
#HC generation + brand 
# df <- df %>% 
...
hist(df$RT) #definitely log-normal! 
hist(df$overall_accuracy)
...
```{r}
dfoutlier <- df %>% 
  mutate(RT = ifelse(RT >= 830, 830, RT)) #define an upper limit of 10 minutes 

write.csv(df, "clean_bachelor.csv")

look <- read.csv("clean_bachelor.csv")
...
```{r}
#density distributions for time  
